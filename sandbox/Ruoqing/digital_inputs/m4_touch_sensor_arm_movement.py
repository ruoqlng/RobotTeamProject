...
def arm_up(arm_motor, touch_sensor):
    ...
    arm_motor.run_to_rel_pos(position_sp=14.2, speed_sp=100)
...
def arm_down(arm_motor):
    ...
    position=-14.2
    arm_motor.run_to_abs_pos(position_sp=position,speed_sp=100)
    arm_motor.wait_while(arm_motor.STATE_HOLDING)  # Blocks until the motor finishes running
    ev3.Sound.beep().wait()
